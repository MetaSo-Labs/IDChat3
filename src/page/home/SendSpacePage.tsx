...
export default function SendSpacePage() {
  ...
  const sendSpace = async () => {
    ...
    const sentRes = await wallet
      .send(inputAddress, amount, { noBroadcast: true })
      .catch((err) => {
        console.log(err);
        ToastView({ text: err.toString(), type: "error" });
      });
  ...
  async function sendHotBtcTest(
    ...
    const utxos = await getBtcUtxos(hotWallet.getAddress(), needRawTx);

    const result = hotWallet.buildTx(SignType.SEND, {
      utxos,
      feeRate: feeRate,
    ...
    // parseToSat
    // const result= hotWallet.signTx(SignType.SEND,{
    //   amount: sendAmount,
    //   utxos,
    //   feeRate: feeRate,
    //   recipient:receiveAddress,
    //   changeAddress:btcWallet.getAddress()
    // });
    // console.log(result);
    ...
    >
      <SafeAreaView style={{ flex: 1 }}>
       ...
       <Text style={{ marginTop: 20, color: "#666", fontSize: 14 ,width:'100%',textAlign:'right'}}> Balance: 0.003234 SPACE</Text>
              ...
              >
                <TextInput
                  placeholder={metaletWallet.currentMvcBalance + " SPACE"}
                  value={inputAmount}
                  onChangeText={(text) => {
