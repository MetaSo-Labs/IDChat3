...
export default function WebsPage(props) {
  ...
  useEffect(() => {
    ...
    // if (!url) {
      ...
      // if (props.route.url && props.route.params.url !== uri) {
     if (url && url !== uri) {
      setURI(props.route.params?.url);
      setReadyURI(props.route.params?.url);
      setIcon(props.route.params.icon);
    }
    }
    initData();
  ...
  async function initData() {
    const isShowNotice = await AsyncStorageUtil.getItem(no_notice_key);
    // const isShowNotice = false;
    console.log("isShowNotice", isShowNotice);

    setIsAgree(isShowNotice);
  }
  ...
  const handlerMessage = async (
    ...
    if (actionType === ActionType.Authorize) {
      ...
      const { title, descriptions, component, needEstimated } =
        actionDispatcher(actionName, actionType);
      // console.log(
      //   "actionMsg2 执行333 ",
      //   title,
      //   descriptions,
  ...
  const confirmAction = async () => {
    try {
      ...
      if (actionType === ActionType.Authorize && needEstimated) {
        ...
        const { nextComponent, estimate } = actionDispatcher(
          actionName,
        ...
        try {
          ...
          const { process } = actionDispatcher(
            actionName,
    ...
    >
      <SafeAreaView style={{ flex: 1 }}>
        ...
        <Modal animationType="slide" visible={modalVisible}>
          <SafeAreaView
            style={{ flex: 1, margin: Platform.OS === "ios" ? 30 : 0 }}
          >
            <View
              style={{
                flex: 1,
        ...
        {Platform.OS === "web" ? (
          ...
          >
            <Text style={{ marginBottom: 16 }}>
              GetWebView does not support on web
            </Text>
          ...
          <WebView
            ...
            onLoadEnd={() => console.log("WebView end")}
            ...
            userAgent={
              Platform.OS === "ios" ? "Metalet-iOS" : "Metalet-Android"
            }
