...
export default function PeoEditInfoPage() {
  ...
  async function intUserMetaIDInfo() {
    if (isRegisterMetaID()) {
      ...
      if (isNotEmpty(metaIDInfo.bio)) {
        setProfile(metaIDInfo.bio);
      }
    }
  }
        ...
        <View style={[metaStyles.verMarginContainer, { marginBottom: 50 }]}>
          ...
          <TouchableWithoutFeedback
            onPress={async () => {
              ...
              //  const compressUri = await compressToTarget(selectedImage, 1024);
              // getSizeInKB(compressUri).then((size) => {
              //   console.log('selectedImage size ', size);
              // });
          ...
          <TextInput
            placeholder={name === '' ? t('chat_peo_name') : name}
            // value={'inputAddress'}
            onChangeText={(text) => {
              setName(text);
            }}
