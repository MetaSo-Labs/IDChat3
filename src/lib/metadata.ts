...
import { METAFILE_API_HOST } from "@/queries/hosts";
...
export async function parse(
  ...
  const { hex: metaTxHex } = await metaletApiV4<{ hex: string }>(
    "/mvc/tx/raw"
  ).get({ network, txid });
...
export async function parseLocalTransaction(
  ...
  const outputIndex = outputs.findIndex((output) =>
    output.script.toASM().includes("OP_RETURN")
  );
